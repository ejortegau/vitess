...
func (txe *TxExecutor) markFailed(ctx context.Context, dtid string) {
	...
	conn, _, _, err := txe.te.txPool.Begin(ctx, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
	if err != nil {
...
func (txe *TxExecutor) inTransaction(f func(*StatefulConnection) error) error {
	conn, _, _, err := txe.te.txPool.Begin(txe.ctx, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
	if err != nil {
		return err
	}
	defer txe.te.txPool.RollbackAndRelease(txe.ctx, conn)
