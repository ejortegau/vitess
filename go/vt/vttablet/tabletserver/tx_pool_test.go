...
func TestTxPoolExecuteCommit(t *testing.T) {
	...
	conn, _, _, err := txPool.Begin(ctx, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
...
func TestTxPoolRollbackNonBusy(t *testing.T) {
	...
	conn1, _, _, err := txPool.Begin(ctx, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
	require.NoError(t, err)
	conn2, _, _, err := txPool.Begin(ctx, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
	require.NoError(t, err)
...
func TestTxPoolTransactionIsolation(t *testing.T) {
	...
	c2, _, _, err := txPool.Begin(ctx, &querypb.ExecuteOptions{TransactionIsolation: querypb.ExecuteOptions_READ_COMMITTED}, false, 0, nil, nil, nil)
	require.NoError(t, err)
...
func TestTxPoolAutocommit(t *testing.T) {
	...
	conn1, _, _, err := txPool.Begin(ctx, &querypb.ExecuteOptions{TransactionIsolation: querypb.ExecuteOptions_AUTOCOMMIT}, false, 0, nil, nil, nil)
	require.NoError(t, err)
...
func TestTxPoolBeginWithPoolConnectionError_Errno2006_Transient(t *testing.T) {
	...
	txConn, _, _, err := txPool.Begin(ctx, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
...
func TestTxPoolBeginWithError(t *testing.T) {
	...
	_, _, _, err := txPool.Begin(ctxWithCallerID, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
	require.Error(t, err)
...
func TestTxPoolBeginWithPreQueryError(t *testing.T) {
	...
	_, _, _, err := txPool.Begin(ctx, &querypb.ExecuteOptions{}, false, 0, []string{"pre_query"}, nil, nil)
	require.Error(t, err)
	require.Contains(t, err.Error(), "error: rejected")
	require.Equal(t, vtrpcpb.Code_UNKNOWN, vterrors.Code(err), "wrong error code for Begin error")
}
...
func TestTxPoolCancelledContextError(t *testing.T) {
	...
	// when
	_, _, _, err := txPool.Begin(ctx, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
...
func TestTxPoolGetConnRecentlyRemovedTransaction(t *testing.T) {
	...
	defer db.Close()
	conn1, _, _, _ := txPool.Begin(ctx, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
	id := conn1.ReservedID()
	conn1.Unlock()
	txPool.Close()
...
func TestTxPoolCloseKillsStrayTransactions(t *testing.T) {
	...
	conn, _, _, err := txPool.Begin(context.Background(), &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
	require.NoError(t, err)
	conn.Unlock()
...
func TestTxTimeoutKillsTransactions(t *testing.T) {
	...
	// Start transaction.
	conn, _, _, err := txPool.Begin(ctxWithCallerID, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
	require.NoError(t, err)
	conn.Unlock()
...
func TestTxTimeoutNotEnforcedForZeroLengthTimeouts(t *testing.T) {
	...
	// Start transactions.
	conn0, _, _, err := txPool.Begin(ctxWithCallerID, &querypb.ExecuteOptions{}, false, 0, nil, nil, nil)
	require.NoError(t, err)
...
func TestTxTimeoutReusedReservedConn(t *testing.T) {
	...
	conn1, _, _, err := txPool.Begin(ctxWithCallerID, &querypb.ExecuteOptions{}, false, conn0.ReservedID(), nil, nil, nil)
	require.NoError(t, err)
...
func TestTxPoolBeginStatements(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("%v:%v:readOnly:%v", tc.txIsolationLevel, tc.txAccessModes, tc.readOnly), func(t *testing.T) {
			...
			conn, beginSQL, _, err := txPool.Begin(ctx, options, tc.readOnly, 0, nil, nil, nil)
			if tc.expErr != "" {
				require.Error(t, err)
...
func newTxPoolWithEnv(env tabletenv.Env) (*TxPool, *fakeLimiter) {
	limiter := &fakeLimiter{}
	return NewTxPool(env, limiter, nil), limiter
}
